...
public class CPUImpossibleButton extends RectangleButton {

	public static final Text HOVER_TEXT = new Text(
			"Computer will ALWAYS follow the ball.\nPretty much impossible to beat this one.",
			Pong.getPong().getWidth() / 2 - 297, Pong.getPong().getHeight() - 30);

	public CPUImpossibleButton(float x, float y) {
		this(x, y, DEFAULT_BUTTON_WIDTH, DEFAULT_BUTTON_HEIGHT);
	}

	public CPUImpossibleButton(float x, float y, int width, int height) {
		this(Color.white, Pong.getPong().getDefaultFont().deriveFont(25.0f), x, y, width, height);
	}

	public CPUImpossibleButton(Color c, Font f, float x, float y, int width, int height) {
		super(c, f, x, y, width, height, true, true, "Impossible");
	}

	@Override
	public void mouseClicked(MouseEvent e) {
		if (this.clickable) {
			Pong.getPong().setCurrentState(new SoloGameState(PlayerTwoCPU.Level.IMPOSSIBLE));
